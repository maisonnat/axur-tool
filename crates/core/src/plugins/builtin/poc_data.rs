        ...
        let duration_html = if data.is_dynamic_window {
            format!(
                r#"<div class="flex items-center gap-3 px-4 py-2 bg-zinc-900 border border-zinc-700 rounded-lg">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-5 h-5 text-[#FF4B00]">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span class="text-zinc-300 text-sm">{} · <span class="text-[#FF4B00] font-semibold">Monitoreo Continuo</span></span>
                </div>"#,
...
<div class="relative h-full flex flex-col">
  ...
  <div class="flex items-start justify-between mb-8">
    <div>
      <span class="bg-[#FF4B00] px-4 py-1 text-sm font-bold tracking-wider uppercase">{title_scope}</span>
  ...
  <div class="flex-grow">
    <div class="grid grid-cols-4 gap-6 h-full">
      <!-- Brands -->
      <div class="bg-zinc-900/80 p-8 rounded-xl border border-zinc-800 flex flex-col justify-center items-center text-center hover:border-[#FF4B00]/50 transition-colors">
        <svg fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" class="w-12 h-12 text-[#FF4B00] mb-4">
